...
class myDb:
    ...
    def __init__(user,password,host="127.0.0.1",port=3306):
        self.host=host;
        self.user=user;
        ...
        assert(self.has_mysql(), "Host has no running mysql connection")
        ...
        for t in self.TABLES:
            if not self.is_table(t):
                eval("self.create_" + t + "table()")

## CHECKS
    def is_port_open(self):
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        return sock.connect((self.host,self.port)) == 0
...
## UTIL
    def execute(self, cmd):
        self.results=self.con.cursor().execute(cmd)
        # XXX GET  OUTPUT
